...
- Reworked the communication tool surface around `add_task_note`, `add_constraint`, and `list_relevant_tasks`, removed silent latest-task fallback for ambiguous references, and added communication eval scaffolding for tool choice and spoken-reply quality regression checks.
