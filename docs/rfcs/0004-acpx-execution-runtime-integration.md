# RFC 0004: Synopse x acpx Execution Runtime Integration

This RFC captures a recommended integration direction between `Synopse` and
...
`acpx` can fit under Synopse as execution substrate and what parts of `acpx`
...
- `Synopse` remains the control plane.
- `acpx` becomes the execution substrate.

In practical terms:

- `Synopse` should continue to own `Task`, `ExecutionSession`,
...
- `Synopse` decides what should happen.
- `acpx` makes agent execution happen reliably.

## Why This RFC Exists

Synopse already has a strong high-level split:
