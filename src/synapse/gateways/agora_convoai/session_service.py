...
class AgoraGatewaySessionService:
    ...
    def get_config(self) -> GatewayConfigResponse:
        ...
        if not self._settings.default_app_id:
            missing.append("SYNAPSE_GATEWAY_AGORA_CONVOAI_APP_ID")
        ...
        if not self._settings.app_certificate:
            missing.append("SYNAPSE_GATEWAY_AGORA_CONVOAI_APP_CERTIFICATE")
        if not self._settings.deepgram_api_key:
            missing.append("SYNAPSE_GATEWAY_AGORA_CONVOAI_DEEPGRAM_API_KEY")
        if not self._settings.elevenlabs_api_key:
            missing.append("SYNAPSE_GATEWAY_AGORA_CONVOAI_ELEVENLABS_API_KEY")
        if not self._settings.elevenlabs_voice_id:
            missing.append("SYNAPSE_GATEWAY_AGORA_CONVOAI_ELEVENLABS_VOICE_ID")
