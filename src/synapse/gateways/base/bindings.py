...
from .transport import SynapseGatewayTransport


@dataclass(slots=True)
class ActiveGatewayBinding:
    binding_id: str
...
class GatewayBindingRegistry:
    def __init__(
        self,
        transport: SynapseGatewayTransport,
        speaker: GatewaySpeaker,
    ) -> None:
        self._transport = transport
        self._speaker = speaker
        self._bindings: dict[str, ActiveGatewayBinding] = {}
    ...
    async def finalize(
        ...
        *,
        ...
        async with self._lock:
            ...
            if binding is None:
                raise KeyError(f"Unknown gateway binding: {binding_id}")
            if not runtime_session_id:
                raise MissingRegistrationConfigError(
                    "runtime_session_id is required for gateway binding."
                )
            if any(
    ...
    def get(self, binding_id: str) -> ActiveGatewayBinding | None:
