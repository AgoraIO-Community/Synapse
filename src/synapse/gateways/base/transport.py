...
class SynapseGatewayTransport(Protocol):
...
class HttpSynapseGatewayTransport:
    def __init__(
        self,
        base_url: str,
        *,
    ...
    async def create_session(self) -> str:
        ...
        if not isinstance(session_id, str) or not session_id:
            raise SynapseGatewayError("Synapse session creation returned no session_id.")
        return session_id
    ...
    async def send_message(self, session_id: str, text: str) -> SynapseMessageResult:
        ...
        if not isinstance(reply_text, str):
            raise SynapseGatewayError("Synapse message response returned no reply_text.")
    ...
    async def _recv_json(
        ...
        *,
        ...
        if not isinstance(raw, str):
            raise SynapseGatewayError("Synapse websocket returned a non-text payload.")
        payload = json.loads(raw)
        if not isinstance(payload, dict):
            raise SynapseGatewayError("Synapse websocket returned a non-object payload.")
        return payload
