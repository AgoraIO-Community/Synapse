...
export default function App() {
  ...
  const activeBro: BroCardModel | null = bros.find((bro) => bro.id === activeBroId) ?? null;
  const voiceConnected = voiceSession.phase === "connected";

  const handleBroPressStart = (broId: string) => {
    setActiveBroId(broId);
    setIsTalking(true);
  };

  const handleBroPressEnd = () => {
    setIsTalking(false);
  };
            ...
            <div className="grid min-h-0 flex-1 grid-cols-1 gap-8 overflow-auto px-8 py-8 lg:grid-cols-[minmax(220px,0.56fr)_minmax(840px,1.74fr)] xl:px-10 xl:py-10">
              ...
              <section className="flex items-start justify-stretch lg:pt-4">
                <BrosPanel
                  ...
                  onBroPressStart={handleBroPressStart}
                  onBroPressEnd={handleBroPressEnd}
                />
              </section>
            </div>
          </main>
