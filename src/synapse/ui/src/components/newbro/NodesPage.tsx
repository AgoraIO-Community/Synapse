...
function NodeForm({
  ...
  function selectExecutor(executorType: string) {
    setEnabledExecutors([executorType]);
  }
  ...
  return (
    <div className="space-y-3 rounded-[24px] border border-neutral-200 bg-white px-4 py-4 sm:px-6 sm:py-5">
      ...
      <div className="space-y-2">
        <div className="text-[12px] uppercase tracking-[0.18em] text-neutral-400">Enabled Executor</div>
        <div className="flex flex-wrap gap-2">
          {EXECUTOR_OPTIONS.map((executorType) => (
            <button
              key={executorType}
              type="button"
              onClick={() => selectExecutor(executorType)}
