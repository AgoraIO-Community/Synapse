...
async def test_app_adds_cors_headers_for_configured_origins():
    app = create_app(
        settings=Settings(
            cors_allowed_origins=("https://newbro-ui.vercel.app",),
        )
    )
    ...
    async with AsyncClient(
        ...
        response = await client.options(
            "/api/sessions",
            headers={
                "Origin": "https://newbro-ui.vercel.app",
                "Access-Control-Request-Method": "POST",
            },
        )

    assert response.status_code == 200
    assert response.headers["access-control-allow-origin"] == "https://newbro-ui.vercel.app"
