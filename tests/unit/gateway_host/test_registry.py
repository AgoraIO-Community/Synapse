...
from synapse.gateway_host.registry import create_gateway_module_registry
...
def test_create_gateway_module_registry_loads_modules_lazily(monkeypatch):
    loaded: list[str] = []
    initialized: list[str] = []
    spec = _FakeSpec(slug="fake", load_counter=loaded, init_counter=initialized)

    monkeypatch.setattr(
        "synapse.gateway_host.registry.list_gateway_module_specs",
        lambda: [spec],
    )

    registry = create_gateway_module_registry()
...
def test_create_gateway_module_registry_filters_out_unrequested_modules(monkeypatch):
    loaded: list[str] = []
    initialized: list[str] = []
    fake_specs = [
    ...
    registry = create_gateway_module_registry(["enabled"])
